...
foreach my $number (sort keys %boundingBox) {
	...
	if(($number % 2) == 0) {
		...
		my $url = 'http://www.mapquestapi.com/traffic/v1/incidents?key=Fmjtd%7Cluub2168nu%2Cax%3Do5-96zg9u&boundingBox=$firstBBLat,$firstBBLong,$secondBBLat,$secondBBLong&filters=construction,incidents&inFormat=kvp&outFormat=json'; #sets up url, this needs to be dynamic
